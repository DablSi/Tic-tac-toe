...
void Players_turn(vector<char>& field, char symbol, const string& note) {
    int place = -1;
...
string If_win(const vector<char>& field, const string& x, const string& o) {
...
void Print_field(const vector<char>& field) {
    cout << endl;
    cout << "| ";
    for (int i = 0; i < 9; ++i) {
        cout << field[i] << " | ";
...
int main()
{
    ...
    if (game_mode == "1" || game_mode == "with computer" ||
        ...
        auto x = std::string("You lost!");
        auto o = std::string("You won!");
